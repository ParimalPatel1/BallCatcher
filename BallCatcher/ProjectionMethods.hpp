...
extern int PreFilterSize;
extern const int PreFilterSize_MAX;
extern int PreFilterCap;
extern const int PreFilterCap_MAX;
extern int SADWindowSize;
extern const int SADWindowSize_MAX;
extern int MinDisparity;
extern const int MinDisparity_MAX;
extern int NumberOfDisparities;
extern const int NumberOfDisparities_MAX;
extern int TextureThreshold;
extern const int TextureThreshold_MAX;
extern int UniquenessRatio;
extern const int UniquenessRatio_MAX;
extern int SpeckleWindowSize;
extern const int SpeckleWindowSize_MAX;
extern int SpeckleRange;
extern const int SpeckleRange_MAX;
extern int Disp12MaxDiff;
extern const int Disp12MaxDiff_MAX;
...
public:
    ...
    void Stereo_Init(cv::Size imgsize)
    {
        ...
        // outputs R1, R2, P1, P2
        cv::stereoRectify(
            M1, D1, M2, D2, img_size, R, T, R1, R2, P1, P2, Q, cv::CALIB_ZERO_DISPARITY, -1, img_size, &roi1, &roi2);
    ...
    // given the normalized disparity image (should be normalized ??), now generate threshold the image so only the
    // ball's points remain.
    ...
    std::vector<cv::Point3f> projectTheBall(std::vector<cv::Point3f> inputpoints)
    {
        ...
        return outputpoints;
        // calculate trajectory and landing position // 'FLOOR' HEIGHT from CAMERA, ASSUME CAMERA is LEVEL with the
        // 'FLOOR'
